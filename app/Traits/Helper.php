...
trait Helper{
    ...
    public function  commande(String $commande){
        $data =  system($commande, $retour);
        return $data;
    }
    
    public function serializeJson($json){
       $array = [];
       foreach ($json as $key => $value) {
        if(is_object($value)){
            $array[$key] = $this->serializeJson((array)$value);
           
        }elseif(is_array($value)){
            $array[$key] = $this->serializeJson($value);
        }else{
            $array[$key] = $value;
        }
       }
    ...
    public function execute($valeur){
        $output=null;
        $retval=null;
    ...
    public function array_compare($comparer, $comparant){
        $final = [];
    ...
    public function reduce($str1, $str2)
      {
        return $str1 .", " . $str2;
      }
    
    public function reduce2($str1, $str2){
        return $str1 .", :".$str2;
    }
    public function reduce3($nb1, $nb2){
        // var_dump($nb2);
        // die;
        return [ "data"=>$nb1["data"]+$nb2["data"]];
    }
    public function operation($el){
        return array_reduce($el, "reduce3", ["data"=>0]);
    }
    
    public function clemax($tableau){//tire la clé du plus grand nombre dans les valeurs d'un tableau
        if(is_array($tableau)){
        $maximum=$tableau[0];
        $cle=0;
    ...
    public function clemax_array_assoc($array){
        if(!empty($array)){
    ...
    public function transform_array_assoc_to_index($array){
    ...
    public function remove_element_in_array($cle_element, $array){
        $new_array = array();
        foreach ($array as $key => $value) {
            if($key!=$cle_element){
                $new_array[$key]=$value;
    ...
    // fonction de calcul temps de publication
    public function temp_publication($temp){
    ...
    public function found_cle_array_imbriquee($cle, $array_imbrique){
    $retourne = false;
    ...
    public function type_document($sorte){
    ...
    public function age_maidusa($age_utilisateur){
    ...
    public function ecrire($variable){
        ?><?=$variable; ?><?php 
    }
    
    public function cle_existe($cle, $tableau){
        //debogueur(cle_element($cle, $tableau));
       return array_key_exists($cle, $tableau);
    }
    public function debogueur($variable){
        var_dump($variable);
    }
     
      public function html_filtres($filtre){
        return htmlspecialchars($filtre);
    }
    public function texte_aleatoire($texte){
        return str_shuffle($texte);
    }
    public function remplacer($terme, $remplaceur, $texte){
       return str_replace($terme, $remplaceur, $texte);
    }
    public function echappe_guillemet($texte){
      return remplacer('"', "'", $texte);
    }
    public function filtre_nombre($texte){
        return filter_var($texte, FILTER_SANITIZE_NUMBER_INT);
    }
    public function filtre_html($texte){
       return filter_var($texte, FILTER_SANITIZE_SPECIAL_CHARS);
    }
    public function filtre_email($email){
        return filter_var($email, FILTER_VALIDATE_EMAIL);
    }
    public function existe($variable){
        return isset($variable);
    }
    public function ilya($tableau2, $tableau1){
    ...
    public function longueur_chaine($chaine){
        return strlen($chaine);
    }
    
    public function longueur_chaine_filtre($chaine, $masque){
        return strcspn($chaine, $masque);
    }
    public function compter_tableau($valeur){
        return count($valeur);
    }
    public function filter_tableau($variable){
        return is_array($variable);
    }
    public function filter_int($variable){
        return is_int($variable);
    }
    public function filter_string($variable){
        return is_string($variable);
    }
    public function c_est_un_tableau($variable){
        return is_array($variable);
    }
    public function cle_element($recherche, $tableau){
        return array_search($recherche, $tableau);
    }
    
    public function melanger2tableau($tableau1, $tableau2){
        return array_merge($tableau1, $tableau2);
    }
    public function distribution_frequence($tableau){
        return array_count_values($tableau);
    }
    
    public function ajouter_au_tableau(&$tableau, $valeur){
        return array_push($tableau, $valeur);
    }
    
    public function supprimer_element_par_cle($tableau, $cle){
    ...
    public function is_true($variable){
      if($variable){
        return true;
      }else{return false;}
    }
    public function extraire_tableau_par_cle_assoc($tableau, $cle){
    ...
    public function vide($variable){
        return empty($variable);
    }
    public function trie_aleatoire($tableau, $nombre_element_a_recupere){
      $tableau_a_retourner = array();
    ...
    return $tableau_a_retourner;
     ...
     public function decouper_en_tableau($delimiter, $variable){
        return explode($delimiter, $variable);
    }
    public function taille_image($image){
        if(file_exists($image)){
           return getimagesize($image);
        }else{
          return false; 
        } }
    public function has_array($array){ // cette fonction verifie si un tableau contient un autre tableau et retourne la clé du dernier element tableau trouvé
        $cle = false;
       // $return = false;
    ...
    public function premier_chaine_dans_tableau($tableau){
        $valeur = false;
        for($i=0; $i<count($tableau); $i++){
    ...
    public function puissance($nombre, $exposant){
        return $nombre**$exposant;
    }
    
    public function cle_element_semblable($array, $verificateur){
        $array_retour = array();
        foreach ($array as $key => $value) {
    ...
    public function premier_element($array){
    //if()
       // var_dump($array);
        $compteur = 0;
    ...
    public function dernier_element($array){
    //if()
        $compteur = 0;
    ...
    public function array_ordonne($array){
      $new_array = array();
    ...
    public function n_ieme_chaine_dans_tableau($tableau, $rang){
       $valeur = false;
       $compteur = 0;
        for($i=0; $i<count($tableau); $i++){
            if(filter_string($tableau[$i])){
    ...
    public function get_array_filter($array, $array_cle){
        $new_array = array();
        foreach ($array as $key => $value) {
    ...
    public function renome_cle_tableau($tableau, $tableau_des_cles){
    ...
    public function add_au_tableau($tableau, $element_a_ajouter){
    ...
    public function compare2tableau_associatif($tableau1, $tableau2, $ignore=""){ //les 2 tableaux divent avoir les memes clés
    ...
    public function remove_numeric_keys($array){
        $new_array = array();
        foreach ($array as $key => $value) {
    ...
    public function c_est_un_entier($valeur){
    ...
    public function remove_colonne_in_array($colonne, $array){
        $new_array = array();
        foreach ($array as $key => $value) {
            if($key==$colonne){}else{
                $new_array[$key]=$value;
    ...
    public function array_has_boolean($array){
        $retour = false;
        foreach ($array as $key => $value) {
    ...
    public function extract_boolean_in_array($array){//retourne un booleen qui est dans un tableau cette fonction doit s'utiliser en association avec la fonction  array_has_boolean ex : if(array_has_boolean($tableau)){ $variable = extract_boolean_in_array($tableau);}
    $retour = false;
    foreach ($array as $key => $value) {
        // if(is_bool($value)){
        //     $retour = $value
    ...
    public function limiteur_textes($textes, $nb_caractere){
    ...
    public function tri_rapide(&$tableau, $indicedebut, $indicefin){
    ...
    public function partition($tableau, $debut, $fin){
    $x = $tableau[$fin];
    $i = $debut-1;
    for ($j=$debut; $j < $fin; $j++) { 
        if($tableau[$j]<$x){
    ...
    public function multiple_alternative_interval($valeur, $tableau_de_valeur){ 
    ...
    public function valeur_proches($valeur, $tableau_des_valeurs, $positif=false){
    ...
    public function trie_par_colonne($tableau, $colonne, $ordre){
    ...
    public function recherche_cle_in_array($tableau, $colonne, $valeur){
        foreach ($tableau as $key => $value) {
    ...
    public function taille_en_octet($var){
    ...
    public function compare($tableau1, $tableau2){
        if(empty(array_diff_assoc($tableau1, $tableau2))){
            return true;
        }else{return false;}
    
    }
    public function is_associative_array($tableau){
        $retourne = false;
    foreach ($tableau as $key => $value) {
    ...
    public function transform_dec_en_hexa($texte){
        return bin2hex($texte);
    
    }
    public function transform_hexa_en_dec($texte){
        return hex2bin($texte);
    }
    public function transform_all_hexa_en_dec($donnees, $cle_texte){
        $data = array();
    ...
    public function transform_all_dec_en_hexa($donnees, $cle_texte1, $cle_texte2=""){
        $data = array();
    foreach ($donnees as $key => $value) {
    ...
    public function get_page_name($url){
        $tab = explode("/", $url);
    ...
    public function request_header($url){
        $tab = explode("/", $url);
        $last = explode("?", $tab[compter_tableau($tab)-1]);
       // var_dump($tab);
    ...
    public function get_repertoire($repertoire){
        $dir = $repertoire;
    $fichiers = array() ;
    while( $nom = $dir->read() ) $fichiers[] = $nom ;
    return $fichiers;
    }
    public function impose_order_in_array($array, $colonne_order){//exige un certain ordre dans les colonnes
    ...
    public function substr2($textes, $debut, $fin){
    $textes_a_retourner = '';
    ...
    public function texte_aleatoire_unique(){
        return str_shuffle("abcdefghijklmnopqrstuvwxyz1234567890").''.time();
    }
    public function generate_identifiant(){
